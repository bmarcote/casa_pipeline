...
#
PLOTBANDPASS_REVISION_STRING = "$Id: plotbandpass3.py,v 1.197 2018/03/12 17:47:05 thunter Exp $" 
import pylab as pb
...
def version(showfile=True):
    ...
    """
    myversion = "$Id: plotbandpass3.py,v 1.197 2018/03/12 17:47:05 thunter Exp $" 
    if (showfile):
...
#                                mytime, scansToPlot, scansForUniqueTimes,  # task version
            ...
            if (ctr > fieldIndicesToPlot and bOverlay==False):
                print "multi-field time overlay ***************  why are there more matches (%d) than fields (%d)?" % (ctr,fieldIndicesToPlot)
...
#            print "done"
    ...
    if (vm != '' or mymsmd != ''):
        if (casadef.casa_version >= '4.4' and casadef.casa_version < '4.6'):
            weatherResult = au.getWeather(vis,bestscan,antenna,verbose,vm,mymsmd,obsid=bestscan_obsid)
        else:
...
#        print "Reduce numchan to ", numchan
    ...
    if verbose: au.printNumberOfAtmosphericLayers(result)
...
def getBasebandDict(vis=None, spwlist=[], caltable=None, mymsmd=None):
    ...
    if (type(bbs) == int):  # old datasets will bomb on msmd.baseband()
        return(bbdict)
    needToClose = False
    if (casadef.casa_version >= '4.1.0' and vis is not None):
        if (os.path.exists(vis)):
            if mymsmd is None or mymsmd == '':
                needToClose = True
                mymsmd = au.createCasaTool(msmdtool)
